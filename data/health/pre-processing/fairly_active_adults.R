...
fairly_active_adults <- read_csv(paste0("https://webservices.esd.org.uk/data.csv?metricType=10270&area=",paste(c("E92000001",cipfa$area_code, "E08000009"), collapse = ','),"&period=latest:5&columnGrouping=period&rowGrouping=area&ApplicationKey=",lginform_key)) %>%
  filter(area != "area") %>%
  pivot_longer("2016":"2020", names_to = 'period', values_to = 'value') %>%
